...
if [[ "$delete_data" == false ]]; then
  ...
  info "Configuration, data, backups, and logs were preserved in $SCRIPT_DIR."
  exit 0
fi

if [[ "$assume_yes" == false ]]; then
  [[ -t 0 ]] || die "--delete-data requires an interactive terminal or the explicit --yes option."
  warn "This permanently deletes Task Central's database, backups, logs, and local configuration."
...
for root in (Path("/data"), Path("/backups"), Path("/logs")):
    if not root.is_dir():
        continue
...
tc_compose down --remove-orphans
rm -f "$ENV_FILE"
info "Task Central containers, database files, backups, logs, and local configuration were deleted."
